...
{
    std::cout << "...... MNIST SimpleNet (VART flow) ......" << std::endl;
    ...
    const float data_mean  = 33.3184204;
    const float data_scale = 0.0127279107;

    std::pair<uint32_t, int> job_id;
    for(unsigned int i = 0; i < num; i++){
        // Scaling before input to DPU
        for(int j = 0; j < in_size; j++){
            buf_in.get()[j] = static_cast<int8_t>( ( static_cast<float>(image.get()[in_size*i + j]) - data_mean)*data_scale*scale );
        }
